...
class OCREngine:
    ...
    def __init__(self):
        ...
        default_timeout = "8" if self.is_render else "5"
        try:
            ...
            self.tesseract_timeout = 8 if self.is_render else 5
    ...
    def upscale(self, image):
        ...
        # Render has limited CPU. Avoid doubling a large label image to
        # 3000-4000+ pixels, which is a common cause of Tesseract timeouts.
        if self.is_render:
            target_max = 2400
    ...
    def extract_easyocr(self, image, force=False):
        if not self.use_easyocr and not force:
            return {
                "text": "",
                "words": [],
                "confidence": 0.0,
        ...
        try:
            ...
            for result in results:
                ...
                # A slightly lower floor is used on Render because small
                # printed label text can otherwise be discarded completely
                # after CPU-only EasyOCR detection.
                confidence_floor = (
                    min(float(self.min_confidence), 10.0)
                    if self.is_render
                    else float(self.min_confidence)
                )
    ...
    def extract(self, path):
        ...
        logger.info(
            "OCR input: file=%s size=%sx%s render=%s tesseract_timeout=%ss",
            path,
            original_width,
            original_height,
            self.is_render,
        ...
        # Render: skip Tesseract completely. In production it is repeatedly
        # hitting the subprocess timeout before returning any words. EasyOCR
        # is used as the Render OCR engine instead. Local behaviour is unchanged.
        if self.is_render:
            variants_to_try = []
            logger.info("Render mode: Tesseract bypassed; using EasyOCR fallback")
        else:
        ...
        if self.use_easyocr or self.is_render or (self.is_render and need_easy_fallback):
            # On Render, explicitly force EasyOCR even when the config flag is
            # disabled. The production failure is exactly the case where
            # Tesseract times out and EasyOCR must take over.
            try:
                easy_image = self.upscale(image)

                if self.is_render and need_easy_fallback:
                    easy_variants = [("original", easy_image)]
                    gray = cv2.cvtColor(easy_image, cv2.COLOR_BGR2GRAY)
                    easy_variants.append(("gray", gray))

                    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
                    enhanced = clahe.apply(gray)
                    easy_variants.append(("clahe", enhanced))
                    ...
                    easy_variants.append(("adaptive", adaptive))

                    easy_candidates = []
                    for variant_name, variant_image in easy_variants:
                        try:
                            candidate = self.extract_easyocr(
                                variant_image,
                                force=True,
                            )
                            candidate["_variant"] = variant_name
                            candidate["_ocr_width"] = variant_image.shape[1]
                            candidate["_ocr_height"] = variant_image.shape[0]

                            if candidate.get("words"):
                                easy_candidates.append(candidate)
                                # Stop at the first useful result to protect
                                # Render CPU/time limits.
                                break
                        except Exception as exc:
                            logger.warning(
                                "EasyOCR %s variant failed: %s",
                                variant_name,
                                exc,
                            )
                    ...
                    if easy_candidates:
                        ...
                        logger.info(
                            "Render EasyOCR selected variant=%s words=%d confidence=%.1f",
                            easy_result.get("_variant", "unknown"),
                            len(easy_result.get("words", [])),
                            float(easy_result.get("confidence", 0.0) or 0.0),
                        )
                    else:
                        logger.warning(
                            "Render EasyOCR returned no usable words from any variant"
                        )
                        easy_result = {
                            "text": "",
                            "words": [],
                            "confidence": 0.0,
                            "available": True,
                            "error": "EasyOCR returned no usable text",
                        }
                else:
        ...
        if not mapped_words:
            logger.error(
                "OCR produced ZERO words. Tesseract_error=%s EasyOCR_error=%s",
                best_tesseract.get("error", ""),
                easy_result.get("error", ""),
            )
